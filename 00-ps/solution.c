...
void ps(void)
{
	...
	while ((cur_entry = readdir(proc_dir_struct)) != NULL) {
		...
		if (dirfd < 0) {
            static char path_err[512];
            snprintf(path_err, 512, "%s/%s", PROC_PATH, cur_entry->d_name);
		...
		if (exe_size < 0) {
            static char path_err[512];
            snprintf(path_err, 512, "%s/%s/%s", PROC_PATH, cur_entry->d_name, "exe");
			report_error(path_err, errno);
			continue;
		}
		...
		if (argv_size < 0) {
            static char path_err[512];
            snprintf(path_err, 512, "%s/%s/%s", PROC_PATH, cur_entry->d_name, "cmdline");
			report_error(path_err, errno);
			continue;
		}
		...
		if (envp_size < 0) {
            static char path_err[512];
            snprintf(path_err, 512, "%s/%s/%s", PROC_PATH, cur_entry->d_name, "environ");
			report_error(path_err, errno);
			continue;
		}
